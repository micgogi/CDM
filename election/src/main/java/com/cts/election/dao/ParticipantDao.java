...
import com.cts.election.entity.Participant;
...
public class ParticipantDao {
	...
	public Map<String, Object> getConstituencyStatus(String constituency) {
		...
		int total = ((BigInteger)entityManager.createNativeQuery("select count(associate_id) from master_data where constituency_name='"+constituency+"'")
				.getSingleResult()).intValue()*20;
		List<Object[]> l1 = entityManager.createNativeQuery(
				"select participant.associate_id,master_data.associate_name,completion_status,party_name from participant inner join master_data "
						...
						+ constituency + "' and party_name='Party 1' order by completion_status desc")
				.getResultList();
		l1.stream().forEach(result -> {
			p1.add(new ConstituencyStatus(String.valueOf(result[0]), String.valueOf(result[1]),
					String.valueOf(result[2]),String.valueOf(result[3])));
		});
		List<Object[]> l2 = entityManager.createNativeQuery(
				"select participant.associate_id,master_data.associate_name,completion_status,party_name from participant inner join master_data "
						+ "where participant.associate_id=master_data.associate_id and constituency_name='"
						+ constituency + "' and party_name='Party 2' order by completion_status desc")
				.getResultList();
		l2.stream().forEach(result -> {
			p2.add(new ConstituencyStatus(String.valueOf(result[0]), String.valueOf(result[1]),
					String.valueOf(result[2]),String.valueOf(result[3])));
		});
	...
	public Set<String> getConstituencies(){
		List<String> list= entityManager.createNativeQuery("select constituency_name from master_data").getResultList();
		list.forEach((data)->{
	...
	public Map<String,WinnerStatus> getWinner(){
		...
		list.stream().forEach(constituency->{
			try {
			winner = entityManager.createNativeQuery("select winner from winners where constituency_name='"+constituency+"'")
...
//				int party2percent = (p2.getVotes()*100)/total;
				...
				if(party1percent>party2percent)
					ws = new WinnerStatus(constituency,p1.getPartyName(),url+p1.getPartyName()+"lead.png");
				else if(party2percent>party1percent)
					ws = new WinnerStatus(constituency,p2.getPartyName(),url+p2.getPartyName()+"lead.png");
				else if(party1percent==0)
					ws = new WinnerStatus(constituency,"no result",url+"No_Result.png");
				else
					ws = new WinnerStatus(constituency,"draw",url+"Drawn.png");
				map.put(constituency,ws);
			}
		});
	...
	public PartyStatus getStatus(String party,String constituency) {
		...
		Object male = entityManager.createNativeQuery(
				"select sum(completion_status) as total_votes from participant inner join master_data "
						+ "where participant.associate_id=master_data.associate_id and constituency_name='"+constituency+"' "
								+ "and party_name='"+party+"' and gender='Male'")
				.getSingleResult();
		if(male != null)
			maleVotes = ((BigDecimal)male).intValue();
			
		Object female = entityManager.createNativeQuery(
				"select sum(completion_status) as total_votes from participant inner join master_data "
						+ "where participant.associate_id=master_data.associate_id and constituency_name='"+constituency+"' "
								+ "and party_name='"+party+"' and gender='Female'")
				.getSingleResult();
		if(female != null)
			femaleVotes = ((BigDecimal)female).intValue();
		
		PartyStatus status = new PartyStatus(party,constituency,maleVotes+femaleVotes,maleVotes,femaleVotes);
		return status;
	}
	...
	public List<DailyData> getDailyData(String constituency) {
		Query query = entityManager.createNativeQuery("select * from daily_data where constituency_name='"
							+constituency+"' order by created DESC limit 0,7", DailyData.class);
	...
	public PartyStatus getStatusWithoutGender(String party,String constituency) {
		...
		Object total = entityManager.createNativeQuery(
				"select sum(completion_status) as total_votes from participant inner join master_data "
						+ "where participant.associate_id=master_data.associate_id and constituency_name='"+constituency+"' "
								+ "and party_name='"+party+"'")
				.getSingleResult();
		if(total != null)
			totalVotes = ((BigDecimal)total).intValue();
    ...
    { 
        ...
        // Sort the list 
        Collections.sort(list, new Comparator<Map.Entry<String, WinnerStatus> >() { 
            public int compare(Map.Entry<String, WinnerStatus> o1,  
                               Map.Entry<String, WinnerStatus> o2) 
            { 
	...
	public Object getParticipantDetails(String id){
		return entityManager.createNativeQuery("select participant.associate_id,participant.associate_name,master_data.constituency_name,master_data.party_name,(participant.completion_status*100)/20 from master_data inner join participant on master_data.associate_id=participant.associate_id where master_data.associate_id="+id).getSingleResult();
	}
}
